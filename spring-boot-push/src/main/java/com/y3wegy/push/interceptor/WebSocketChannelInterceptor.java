...
public class WebSocketChannelInterceptor implements ChannelInterceptor {
    ...
    public Message<?> preSend(Message<?> message, MessageChannel messageChannel) {
         ...
         */
        String token = accessor.getNativeHeader(WebConstants.TOKEN).get(0);

        /**
         * 1. 这里直接封装到StompHeaderAccessor 中，可以根据自身业务进行改变
         * 2. 封装到StompHeaderAccessor中后，可以在@Controller / @MessageMapping注解的方法中直接带上StompHeaderAccessor
        ...
        if (StompCommand.CONNECT.equals(accessor.getCommand())) {
            logger.info("认证用户:{} 页面传递令牌:{}", user.toString(), token);
        } else if (StompCommand.DISCONNECT.equals(accessor.getCommand())) {
            logger.info("认证用户:{} disconnect", user.toString());
        }
        return message;
    }
    ...
    public void postSend(Message<?> message, MessageChannel messageChannel, boolean sent) {
         ...
         */
        HttpSession httpSession = (HttpSession) accessor.getSessionAttributes().get(WebConstants.HTTP_SESSION);
        ...
        // 首次连接
        case CONNECT:
            logger.info("httpSession key：{} 首次连接", httpSession.getId());
            break;
        // 连接中
        case CONNECTED:
            break;
        // 下线
        case DISCONNECT:
            logger.info("httpSession key：{} 下线", httpSession.getId());
            break;
        default:
            break;
        }
